...
def is_realestate_update_day():
    # 금(4), 토(5)만 True (한국시간 기준, UTC+9)
    import datetime
    KST = datetime.timezone(datetime.timedelta(hours=9))
    now_kst = datetime.datetime.now(KST)
    return now_kst.weekday() in [4, 5]
...
def generate_static_html(main_only=False, realestate_only=False):
    ...
    if realestate_only:
        ...
        ma_graphs_html = stock_data = economy_news_data = realestate_news_data = ""
    # stock_summary_html = ""
    elif main_only:
    ...
    # 아파트 시세 데이터 수집
    apartment_prices = get_all_apartment_prices()
...
<body>
    ...
    <div id="economy-main-tab" class="main-tab-content active">
        ...
        <!-- 월스트리트저널 나우 바로가기 -->
        <section class="wsj-link-section" style="margin-bottom:24px;">
            <h2 style="font-size:1.1em; margin-bottom:8px;">월스트리트저널 나우</h2>
            <div style="font-size:0.98em; line-height:1.7; background:#f8fafc; border-radius:8px; padding:16px; border:1px solid #e2e8f0;">
                <a href="https://www.hankyung.com/globalmarket/news-wallstreet-now" target="_blank" rel="noopener" style="color:#2563eb; text-decoration:underline; font-size:1em;">
                    월스트리트 저널 나우 바로가기
                </a>
            </div>
        </section>
    ...
    <div id="realestate-map-main-tab" class="main-tab-content">
        <section class="apartment-price-map-section" style="padding-right: 5px;padding-left: 5px;padding-top: 5px;">
            <h2>대장 단지 실거래 시세</h2>
            <div class="naver-map-container">
                <div id="naver-map-apartment-price" style="margin-bottom: 10px; height: 600px;"></div>
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 10px;">
                <small style="font-size: 13px; color: #4a5568;">업데이트: <span id="update-date-apt"></span></small>
            </div>
            <div class="map-info" style="margin-top: 10px;">
                <p>※ 지역 마커를 클릭하면 대장 단지의 매매/전세/월세 실거래 시세를 확인할 수 있습니다</p>
    ...
    window.apartmentPrices = {json.dumps(apartment_prices, ensure_ascii=False, separators=(',', ':'))};
    </script>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {{
            ...
            // 대장 단지 지도 날짜
            const updateDateAptElement = document.getElementById('update-date-apt');
            if (updateDateAptElement) {{
                updateDateAptElement.textContent = dateString;
            }}
        }});
    </script>
